...
class FrameInfo:
    ...
    Attributes:
        ...
        key_frame: True when ffprobe marks this frame as a random-access frame.
        timestamp: Best available timestamp in seconds, or None if missing.
    """
...
class BitstreamAnalyzer:
    ...
    def get_frame_types(
        ...
        for idx, pkt in enumerate(raw):
            ...
            try:
                # pkt_pts_time is common for MP4; best_effort_timestamp_time is
                # more reliable for codecs with frame reordering (B-frames).
                timestamp = _to_float(
                    pkt.get("best_effort_timestamp_time")
                    or pkt.get("pkt_pts_time")
                    or pkt.get("pts_time")
                )
                pts_time = timestamp if timestamp is not None else 0.0
                ...
                key_frame = str(pkt.get("key_frame", "0")) == "1"
            except (ValueError, TypeError):
                # Corrupted or missing fields - use safe zero defaults.
                pts, dts, pts_time, size, key_frame, timestamp = 0, 0, 0.0, 0, False, None
    ...
    def summarize(self, encoded_path: str | Path) -> dict[str, Any]:
        ...
        if not frames:
            metadata = self.get_stream_metadata(encoded_path)
            stream_frame_count = metadata.get("frame_count") or 0
            return {
                "total_frames": stream_frame_count,
                "frame_count": stream_frame_count,
                "keyframe_count": 0,
                "frame_type_counts": {"I": 0, "P": 0, "B": 0, "other": 0},
                "gops": [],
                "gop_count": 0,
                "gop_lengths": [],
                "gop_min": None,
                ...
                "total_size_bytes": 0,
                "issues": [],
                "warnings": ["Frame-level metadata unavailable; using stream-level frame count"],
            }
        ...
        other_count = len(frames) - i_count - p_count - b_count
        keyframe_count = sum(1 for f in frames if f.key_frame or f.frame_type == "I")
        ...
        pts_vals = [f.timestamp for f in frames if f.timestamp is not None]
        ...
        return {
            ...
            "gop_min": min(gop_lengths) if gop_lengths else None,
            "gop_max": max(gop_lengths) if gop_lengths else None,
            "gop_avg": (sum(gop_lengths) / len(gop_lengths)) if gop_lengths else None,
    ...
    def _probe_frames(self, encoded_path: str | Path) -> list[dict]:
        ...
        cmd = [
            ...
            "-show_entries",
            "frame=key_frame,pict_type,best_effort_timestamp_time,pkt_pts_time,pts,pts_time,pkt_dts,pkt_size",
            "-of", "json",
            str(encoded_path),
        ]
        try:
        ...
        # With -select_streams v:0 and explicit -show_entries, ffprobe may omit
        # media_type entirely. Accept those records as video frames; only discard
        # entries that explicitly identify a non-video stream.
        return [
            f for f in data.get("frames", [])
            if f.get("media_type", "video") == "video"
        ]
