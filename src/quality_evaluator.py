...
class QualityEvaluator:
    ...
    def compute_vmaf(
        ...
        filter_str = (
            ...
            f"[dist][ref]libvmaf=log_path={self._filter_path(json_path)}:log_fmt=json"
    ...
    def compute_psnr(
        ...
        filter_str = (
            f"[0:v]setpts=PTS-STARTPTS[ref];"
            f"[1:v]setpts=PTS-STARTPTS[dist];"
            f"[dist][ref]psnr=stats_file={self._filter_path(stats_path)}"
        )
    ...
    def compute_ssim(
        ...
        filter_str = (
            f"[0:v]setpts=PTS-STARTPTS[ref];"
            f"[1:v]setpts=PTS-STARTPTS[dist];"
            f"[dist][ref]ssim=stats_file={self._filter_path(stats_path)}"
        )
    ...
    def _filter_path(self, path: str | Path) -> str:
        """Return a path string safe for FFmpeg filter option values.

        FFmpeg filter arguments treat backslashes as escapes, so Windows-style
        paths such as ``outputs/metrics/file.log`` can silently collapse into
        ``outputsmetricsfile.log``. Forward slashes keep relative paths portable
        in PowerShell, Git Bash, and FFmpeg.

        Args:
            path: Path used inside a filter option such as stats_file or log_path.

        Returns:
            str: POSIX-style path with drive-colon escaping for absolute paths.
        """
    ...
    def _parse_ssim_log(self, stats_path: Path, stderr: str) -> dict:
        ...
        for line in reversed(stderr.splitlines()):
            if "SSIM" in line and "All:" in line:
                match = re.search(r"All:([0-9]*\.?[0-9]+)", line)
                if match:
                    return {
                        "ssim_y": float("nan"), "ssim_u": float("nan"),
                        "ssim_v": float("nan"),
                        "ssim_avg": float(match.group(1)), "frame_count": 0,
                    }
