...
def test_parse_ssim_stderr_extracts_all_value(tmp_path):
    """SSIM parser should read FFmpeg stderr summary lines with All:<value>."""
    evaluator = QualityEvaluator(metrics_output_dir=tmp_path)
    stderr = "[Parsed_ssim_0 @ 000] SSIM Y:0.987 U:0.991 V:0.992 All:0.988 (19.2)"
    result = evaluator._parse_ssim_log(tmp_path / "missing_ssim.log", stderr)
    assert result["ssim_avg"] == pytest.approx(0.988)


def test_filter_path_uses_forward_slashes_for_ffmpeg_filters(tmp_path):
    """Metric filter paths should not collapse Windows backslashes in FFmpeg."""
    evaluator = QualityEvaluator(metrics_output_dir=tmp_path)
    path = evaluator._filter_path(Path("outputs") / "metrics" / "clip_ssim.log")
    assert path == "outputs/metrics/clip_ssim.log"
