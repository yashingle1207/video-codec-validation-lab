...
def test_evaluate_thresholds_skips_optional_missing_vmaf():
    """Missing VMAF should not fail unless the threshold marks it required."""
    passed, failures = evaluate_thresholds(
        {"psnr_y": 40.0, "vmaf": None},
        {"min_psnr_y": 36.0, "min_vmaf": 85.0},
    )
    assert passed is True
    assert failures == []


def test_evaluate_thresholds_fails_required_missing_vmaf():
    """A required VMAF rule should fail when the metric is unavailable."""
    passed, failures = evaluate_thresholds(
        {"vmaf": None},
        {"min_vmaf": 85.0, "required_vmaf": True},
    )
    assert passed is False
    assert failures == ["vmaf=None missing required metric"]
